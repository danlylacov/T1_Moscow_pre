#!/bin/bash
# Скрипт для тестирования реальных приложений

API_URL="http://localhost:8001/analyze"
RESULTS_DIR="/home/daniil/Рабочий стол/T1/test_repos/results_real_apps"
mkdir -p "$RESULTS_DIR"

# Список реальных приложений
REPOS=(
    # Java проекты
...
echo "=== Тестирование реальных приложений ==="
...
for i in "${!REPOS[@]}"; do
    ...
    if [ "$http_code" = "200" ]; then
        if echo "$body" | python3 -m json.tool > /dev/null 2>&1; then
            ...
            test_runners=$(echo "$body" | python3 -c "import sys, json; d=json.load(sys.stdin); print(', '.join(d.get('test_runner', [])))" 2>/dev/null)
            ...
            echo "   Test Runners: ${test_runners:-None}"
